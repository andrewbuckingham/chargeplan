...
public record AdvancedAlgorithmBuilder(IPlant PlantTemplate,
    ...
    AlgorithmPrecision AlgorithmPrecision) : AlgorithmBuilder(PlantTemplate,
        ...
        AlgorithmPrecision)
{
    public AdvancedAlgorithmBuilder AddChargeWindow(PowerAtAbsoluteTimes template, DateTime day) => this with { FixedChargeProfile = (FixedChargeProfile ?? throw new InvalidOperationException("Already chosen to use dynamic charge windows")).Add(template.AsChargeProfile(day.Date)) };
